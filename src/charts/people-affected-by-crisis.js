...
const matchCountryNames = (csvData, worldData) => {
  const matchedData = csvData.map((stream) => {
    const countryObject = worldData.features.find((feature) => feature.properties.iso_a3 === stream.Country_ID);
    if (countryObject) {
        ...
        Array.prototype.forEach.call(chartNodes, (chartNode) => {
          ...
          fetchCSVData(CSV_PATH).then((data) => {
            ...
            window.$.getJSON(MAP_FILE_PATH, (worldJson) => {
              ...
              const countries = [...new Set(procesedCountryNameData.map((stream) => stream.Country_name))];
