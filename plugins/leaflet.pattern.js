...
!(function (t, e) {
  (L.Pattern = L.Class.extend({
    ...
    onAdd(t) {
      ...
      for (const e in this._shapes) this._shapes[e].onAdd(this);
      this._addShapes(),
        this._addDom(),
        this.redraw(),
    ...
    redraw() {
      if (this._map) {
        this._update();
        for (const t in this._shapes) this._shapes[t].redraw();
      }
      ...
      _updateStyle() {
        ...
        if (t) {
          if (
            ...
            let i = e.patternTransform ? `${e.patternTransform} ` : '';
            (i += e.angle ? `rotate(${e.angle}) ` : ''), t.setAttribute('patternTransform', i);
          } else t.removeAttribute('patternTransform');
          for (const s in this._shapes) this._shapes[s]._updateStyle();
        }
      },
    })),
    L.Map.include({
